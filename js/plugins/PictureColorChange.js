...
if (isHueChanged || isTintChanged || isPictureChanged) {
  ...
  this.bitmap.addLoadListener(function () {
    ...
    // nullチェック & 読み込み確認
    if (!self.bitmap || !self.bitmap.isReady()) {
      console.warn("bitmap未準備のまま処理が呼び出されました");
      return;
    }
    if (!self.bitmap.width || !self.bitmap.height) {
      console.warn("bitmap.width/heightがnull");
      return;
    }
    ...
    if (isPictureChanged) {
      // 未加工のbitmapを保存しておく
      self.defaultBitmap = self.bitmap;
    }

    var bitmap = Object.create(Bitmap.prototype);
    bitmap.initialize(self.defaultBitmap.width, self.defaultBitmap.height);
    bitmap.smooth = true;
    bitmap.blt(self.defaultBitmap, 0, 0, self.defaultBitmap.width, self.defaultBitmap.height, 0, 0);
    self.bitmap = bitmap;

    if (isTintChanged || self.__tint != 0xFFFFFF) {
      self.bitmap.setTint(self.__tint);
    }
    if (isHueChanged || self.__hue != 0) {
      self.bitmap.rotateHue(self.__hue);
    }
  }.bind(this));
}}
