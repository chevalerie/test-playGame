...
Game_System.prototype.addMessageBacklog = function(text) {
  ...
  // 直前のメッセージと同じなら追加しない
  if (this._messageBacklog.length > 0 && this._messageBacklog[this._messageBacklog.length - 1] === text) {
    return;
  }
...
Window_MessageBacklog.prototype.removeMessageBoxOnlyCodes = function(text) {
  ...
  text = text.replace(/\\cv\[[^\]]+\]/g, '');/*チャットgptで生成したプラグインで使う「\cv[～～]」という制御文字をバックログに表示させないために追加したもの*/
...
Yanfly.MsgBacklog.SceneManager_push = SceneManager.push;
SceneManager.push = function(sceneClass) {
  if (sceneClass === Scene_Save) {
    $gameSystem.setMessageBacklogLoggingEnable(false);
  }
  Yanfly.MsgBacklog.SceneManager_push.call(this, sceneClass);
};

Yanfly.MsgBacklog.SceneManager_goto = SceneManager.goto;
SceneManager.goto = function(sceneClass) {
  if (sceneClass === Scene_Map) {
    $gameSystem.setMessageBacklogLoggingEnable(true);
  }
  Yanfly.MsgBacklog.SceneManager_goto.call(this, sceneClass);
};
